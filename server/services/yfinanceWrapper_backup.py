...
def get_stock_data(symbol, retry_count=0, max_retries=2):
    """Fetch comprehensive stock data for a given symbol with retry logic"""
    try:
        print(f"[DEBUG] Creating ticker object for {symbol} (attempt {retry_count + 1}/{max_retries + 1})", file=sys.stderr)
...
if __name__ == "__main__":
    try:
        if len(sys.argv) > 1:
            ...
            if ',' in arg:
                # Batch mode: "AAPL,MSFT,GOOGL"
                symbols = [s.strip().upper() for s in arg.split(',')]
                print(f"[DEBUG] Fetching batch data for symbols: {symbols}", file=sys.stderr)
                data = get_stock_data_batch(symbols)
            else:
                # Single mode: "AAPL"
                symbol = arg.upper()
                print(f"[DEBUG] Fetching data for symbol: {symbol}", file=sys.stderr)
                data = get_stock_data(symbol)
            
            print(f"[DEBUG] Data fetch completed, cleaning NaN values", file=sys.stderr)
            data = clean_nan_values(data)
            print(json.dumps(data, indent=2))
        else:
            error_msg = {"error": "Symbol required"}
...
def get_stock_data_batch(symbols):
    """
    Fetch comprehensive stock data for multiple symbols in a single batch
    
    Args:
        symbols: List of ticker symbols (e.g., ["AAPL", "MSFT", "GOOGL"])
    
    Returns:
        Dict of {symbol: financial_data} for all symbols
    """
    ...
    try:
        ...
        for symbol in symbols:
            try:
                # Get ticker object from batch
                ticker = tickers.tickers.get(symbol)
                
                if ticker is None:
                    results[symbol] = {"error": f"Ticker {symbol} not found", "symbol": symbol}
                    continue
                
                # Use existing get_stock_data logic but with pre-fetched ticker
                # Get current quote
                info = ticker.info
                ...
                try:
                    ...
                    if ttm_data["revenue"] > 0 or ttm_data["operating_income"] > 0:
                        result["financials"].append({
                            "period": "TTM",
                            "fiscalYear": datetime.now().year,
                            "revenue": (ttm_data["revenue"] / 1e9) * conversion_rate,
                            "netIncome": (ttm_data["net_income"] / 1e9) * conversion_rate,
                            "eps": float(info.get('trailingEps', 0)) or 0,
                            "operatingIncome": (ttm_data["operating_income"] / 1e9) * conversion_rate,
                            "freeCashFlow": (ttm_data["fcf"] / 1e9) * conversion_rate
                        })
        ...
        error_str = str(e)
        print(f"[ERROR] Exception in get_stock_data for {symbol}: {error_str}", file=sys.stderr)
        
        # Retry on connection errors
        if retry_count < max_retries and ('Connection' in error_str or 'curl' in error_str or 'timeout' in error_str.lower()):
            print(f"[INFO] Retrying {symbol} due to connection error...", file=sys.stderr)
            import time
